...
###   This script attempts to download the signature file SHA256SUMS.asc from aureus.org
...
WORKINGDIR="/tmp/aureus"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://aureus.org/bin/"
VERSIONPREFIX="aureus-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'aureus-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Aureus Core binary release signing key installed?"
   fi
