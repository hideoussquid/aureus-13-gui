...
The RPM spec file provided here is for Aureus-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://aureus.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Aureus git repository](https://github.com/aureus/aureus) but are not part of
...
Currently only `Source10` is used, for the example `aureus.conf` file.
...
in the aureus git repository. At present only an SVG version of the Aureus
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the aureus
...
needed when 0.12.1 is released, a proper fix is already in the Aureus git
master branch.
...
idea to build Aureus Core against a static version of that library compiled
...
The `aureus.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `aureus-server` package. At this point, most RPM based
...
The `aureus-server` package should have SELinux support. How to properly do
...
Aureus-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/aureus/). He has been
packaging Aureus for Fedora at least since 2012.
...
4. I renamed the `aureus` package that contains the Qt GUI to `aureus-core` as
that appears to be how the general population refers to it, in contrast to
`aureus-xt` or `aureus-classic`. I wanted to make sure the general population
